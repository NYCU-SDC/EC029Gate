...
function AccessControl() {
	...
	const loadData = async () => {
		setLoading(true);
		try {
			const [guildsRes, rolesRes] = await Promise.all([fetch("/api/admin/discord/guilds", { credentials: "include" }), fetch("/api/admin/roles", { credentials: "include" })]);
	...
	return (
		<div className="access-control">
			<div className="card">
				...
				<p className="section-description">Users with the selected role will have access to unlock the door.</p>
				...
				<button className="btn btn-primary" onClick={handleAddRole} disabled={!selectedGuild || !selectedRole}>
					Add Role
				</button>
			</div>
						...
						{allowedRoles.map(role => {
							...
							return (
								<div key={`${role.guild_id}-${role.role_id}`} className="role-item">
									...
									<button className="btn btn-danger btn-sm" onClick={() => handleRemoveRole(role.guild_id, role.role_id)}>
										Remove
									</button>
								</div>
							);
